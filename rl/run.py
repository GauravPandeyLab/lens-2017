...
def main(argv):
    ...
    conv 	= int(args.conv)
    algo 	= args.algo
    assert strategy in ['greedy', 'pessimistic', 'backtrack']
    assert algo     in ['Q']
    ...
    world = Environment.World(num_pred, start, exit, in_dir, seed, fold, rule, metric)
    instruction = ("agent = selection.Agent(epsilon, seed, strategy, age, conv)")
    exec(instruction) 
    ...
    result   = 'Fold_%i age=%s num_episodes=%i (visited=%f%% (%i nodes), explored=%f%% (%i nodes)) (val=%f) (test=%f) (bestBP %s=%f) (FE=%f) :: %r [%s]\n%s'  % (fold, agent.age, agent.num_episodes, float(len(agent.getVisitedStates()))/float(2**num_pred)*100, len(agent.getVisitedStates()), float(len(agent.getExploredStates()))/float(2**num_pred)*100, len(agent.getExploredStates()), agent.getEnsPerf(), agent.getTestPerf(ensemble), agent.world.getBestBP(), agent.getBestBPPerf_onTest(), agent.getFEPerf_onTest(), ensemble, time.strftime('%H:%M:%S', time.gmtime(seconds)), bps2string(list(agent.world.predictors)))
